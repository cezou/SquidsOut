...
/*   Updated: 2024/03/06 16:36:01 by cviegas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	g->draw.p_new_pos = faddmul(g->draw.p_pos, g->draw.p_vel, 0.05);
	avoid_theses_tiles("1", g);
	collect(g);
	g->draw.p_pos = g->draw.p_new_pos;
	if (!g->draw.p_vel[0] && !g->draw.p_vel[1])
		return ;
	g->tiles_traveled[0] += fabs(g->draw.p_vel[0]) * 0.05;
	g->tiles_traveled[1] += fabs(g->draw.p_vel[1]) * 0.05;
}
